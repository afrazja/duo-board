...
create table if not exists public.messages (
  ...
  stopped_for  text[] not null default '{}' check (stopped_for <@ array['claude', 'chatgpt']::text[])
);
...
alter table public.messages add column if not exists stopped_for text[] not null default '{}';
...
begin
  ...
  where m.id = p_message_id and m.thread_id = p_thread_id
    and m.author = 'user' and m.addressed_to in ('both', p_assistant)
    and exists (select 1 from public.threads t where t.id = p_thread_id and t.owner_id = p_owner_id)
  returning m.* into stopped_message;
  if not found then raise exception 'Task not found'; end if;
  update public.assistants set working_on_seq = null
  where owner_id = p_owner_id and name = p_assistant and working_on_seq = stopped_message.seq;
