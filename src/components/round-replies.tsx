  ...
  return <div className="space-y-4">
    ...
    {revealed && compare && <section aria-label="Answer comparison" className="rounded-xl border border-indigo-500/30 bg-indigo-500/5 p-4">
      ...
      <div className="grid min-w-0 grid-cols-1 gap-3 lg:grid-cols-2">{(["claude", "chatgpt"] as const).map((who) => <div className="min-w-0 space-y-3" key={who}>
        {compare[who].length ? <ReplyList messages={compare[who]} who={who} askedAt={compare.request.created_at} playback={playback} compact={false} /> : <Waiting helper={helper} onWake={onWake} waking={waking} who={who} question={compare.request} now={now} paused={paused} stopping={stopping.includes(`${who}:${compare.request.id}`)} onStop={onStop ? () => onStop(compare.request, who) : undefined} status={assistants.find((a) => a.name === who)} />}
      </div>)}</div>
    </section>}
