...
type Dependencies = {
  ...
  prepareReceive?: (ownerId: string, result: Record<string, unknown>) => Promise<Record<string, unknown>>;
  afterResult?: (requestId: string) => Promise<void>;
  waitMs?: number;
};
...
const capabilities = z.array(z.literal("local_transcription")).max(1).optional();
const requestSchema = z.object({ id, thread_id: id, action: z.enum(["wake", "message", "stop", "activity", "transcribe"]), message_id: id.optional(), assistant: assistant.optional(), audio_path: z.string().max(300).optional(), audio_sha256: z.string().regex(/^[a-f0-9]{64}$/).optional(), audio_bytes: z.number().int().min(44).max(20 * 1024 * 1024).optional() }).strict()
  .refine((r) => (r.action !== "message" || Boolean(r.message_id)) && (!["activity", "transcribe"].includes(r.action) || !r.message_id) && (r.action !== "transcribe" || Boolean(r.audio_path && r.audio_sha256 && r.audio_bytes)));
const deviceSchema = z.discriminatedUnion("action", [
  z.object({ action: z.literal("receive"), instance_id: id, conversations, assistants, capabilities, wait: z.boolean().optional(), workspace_cleanup: z.boolean().optional() }).strict(),
  z.object({ action: z.literal("ack"), instance_id: id, conversations, assistants, capabilities, id, rejected: z.boolean().optional() }).strict(),
  z.object({ action: z.literal("result"), instance_id: id, conversations, assistants, capabilities, id, status: z.enum(["completed", "stopped", "failed", "attention"]), result: z.string().max(1_000_000).nullable().optional(), error: z.string().max(1000).nullable().optional() }).strict(),
]);
...
export function helperHandlers({ rpc, requireUser, listRemovedThreads, prepareReceive, afterResult, waitMs = 15_000 }: Dependencies) {
  ...
  return {
    ...
    device: (request: Request) => protect(async () => {
      ...
      for (;;) {
        ...
        if (action !== "receive" || (result.requests as unknown[]).length || (result.removed_thread_ids as string[] | undefined)?.length || Date.now() >= until || request.signal.aborted) {
          const response = action === "receive" && prepareReceive ? await prepareReceive(String(result.owner_id), result) : result;
          if (action === "result" && afterResult) await afterResult(args.id).catch(() => {});
