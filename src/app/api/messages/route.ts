...
export async function PATCH(req: Request) {
  try {
    ...
    if (typeof body.thread_id !== "string" || typeof body.message_id !== "string" || (body.assistant !== "chatgpt" && body.assistant !== "claude")) {
      return Response.json({ error: "thread_id, message_id and the assistant to stop are required" }, { status: 400 });
    }
    const message = await stopAssistantTask(user.id, body.thread_id, body.message_id, body.assistant);
    return Response.json({ message });
  } catch (e) {
    return authErrorResponse(e) ?? Response.json({ error: (e as Error).message }, { status: 400 });
  }
