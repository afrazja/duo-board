    ...
    (server) => {
      ...
      server.registerTool(
        "post_message",
        {
          ...
          description: `Post your reply into a thread as ${who}. Write the complete answer in Markdown in body. Set reply_to to the id of the person's message you are answering: that is how the board knows your answer is in and can release ${other}'s answer to you. A progress note that is not your answer should not set reply_to to the question. Optionally provide a short spoken_summary for Brief audio. When voice_mode is true: ${BRIEF_AUDIO_GUIDANCE} Answer only what was addressed to you or to both; when the person addressed the other assistant alone, read but do not post. A paused conversation rejects assistant posts until it is resumed. An answer to a message the person stopped for you is refused; drop that task.`,
          inputSchema: z.object({
      ...
      server.registerTool(
        ...
        {
          title: "Read a thread",
          description: `The latest messages of one thread, oldest first, for context. Does not move your read cursor. For a question with blind_round true (or absent on older messages), the blind first round applies here too: ${other}'s answer to a question you have not answered yet is left out. A message the person stopped for you carries stopped_for_you: true; do not answer it.`,
