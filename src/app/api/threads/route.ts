...
export async function PATCH(req: Request) {
  try {
    const user = await requireUser();
    const parsed = z.object({ thread_id: z.string().uuid(), title: z.string().trim().min(1).max(120).optional(), brief_audio: z.boolean().optional(), blind_first_round: z.boolean().optional(), paused: z.boolean().optional() })
      .refine((value) => value.title !== undefined || value.brief_audio !== undefined || value.blind_first_round !== undefined || value.paused !== undefined).safeParse(await req.json());
    if (!parsed.success) return Response.json({ error: "A valid thread_id and conversation change are required" }, { status: 400 });
