...
export class StateStore {
  ...
  async load() {
    ...
    for (const [key, c] of Object.entries(s.conversations)) {
      ...
      if (c.workspaceRename !== undefined && (!c.workspaceRename || typeof c.workspaceRename.from !== "string" || !path.isAbsolute(c.workspaceRename.from) || typeof c.workspaceRename.to !== "string" || !path.isAbsolute(c.workspaceRename.to))) throw new Error("Invalid workspace move journal; recovery is required.");
      if (c.workspaceMovedAt !== undefined && (typeof c.workspaceMovedAt !== "string" || !Number.isFinite(Date.parse(c.workspaceMovedAt)))) throw new Error("Invalid workspace move state; recovery is required.");
      if (!validText(c.workspaceNameError, 2000) || (c.workspaceNameRetryAt !== undefined && (!Number.isFinite(c.workspaceNameRetryAt) || c.workspaceNameRetryAt < 0))) throw new Error("Invalid workspace naming retry state.");
