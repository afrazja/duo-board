...
The Windows helper creates one Codex task for each new board conversation, stores that exact conversation-to-task mapping locally, and resumes only that task for later messages in the same conversation. It uses one authenticated background connection for all conversations, so no `/loop` command is needed in each Codex task. Each workspace is created under `Documents/Duo Board Workspaces/<conversation UUID>`. Apply `supabase/helper-thread-tasks.sql` after the helper migrations on an existing database so new conversations create their Codex tasks immediately.

The same helper does this for Claude when Claude Code is installed on that computer. Each new conversation gets its own Claude Code session, reserved by the helper under a fixed session ID and created on the conversation's first Claude-addressed message with `claude -p --session-id`; later messages continue it with `--resume`, and the session is named `Duo Board — <conversation title>`. The board shows **Own Claude session · <short-id>** beside **Own Codex task · <short-id>**. A question to both assistants becomes two requests, one per assistant, answered side by side in their own sessions; Stop in a card stops only that assistant. Claude Desktop offers no supported way to create or continue its sessions from another program, so the helper uses the Claude Code CLI, whose headless mode is the documented interface for exactly this; the sessions it creates can also be opened later with `claude --resume <id>` from the conversation's workspace folder. Without Claude Code the helper manages ChatGPT only, and Claude keeps the dispatcher pattern below: one session that lists threads each tick and hands each conversation to its own long-lived subagent with an empty context.
...
While a ChatGPT card, or a Claude card on a helper-managed account, says **Waiting for an answer** or **Working on an answer**, the person can select **Stop** in that card. The linked answer is then rejected even if it was already being composed, while the other assistant and later questions continue normally. Apply `supabase/task-stops.sql` once on an existing database.
